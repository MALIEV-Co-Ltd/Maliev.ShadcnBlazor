...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.1.0.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.1.0.snupkg");
            ...
            Assert.Equal("1.1.0", metadata.Element(ns + "version")!.Value);

...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.4.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.4.snupkg");
            ...
            Assert.Equal("1.0.4", metadata.Element(ns + "version")!.Value);

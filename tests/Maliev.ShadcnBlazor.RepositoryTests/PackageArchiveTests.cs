...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.10.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.10.snupkg");
            ...
            Assert.Equal("1.0.10", metadata.Element(ns + "version")!.Value);

...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.9.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.9.snupkg");
            ...
            Assert.Equal("1.0.9", metadata.Element(ns + "version")!.Value);

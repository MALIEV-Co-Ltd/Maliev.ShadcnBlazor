...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.2.1.3.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.2.1.3.snupkg");
            ...
            Assert.Equal("2.1.3", metadata.Element(ns + "version")!.Value);

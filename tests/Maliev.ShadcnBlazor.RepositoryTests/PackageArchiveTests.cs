...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.5.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.5.snupkg");
            ...
            Assert.Equal("1.0.5", metadata.Element(ns + "version")!.Value);

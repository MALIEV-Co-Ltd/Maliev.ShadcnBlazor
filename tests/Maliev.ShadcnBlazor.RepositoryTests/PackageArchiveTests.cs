...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.2.1.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.2.1.snupkg");
            ...
            Assert.Equal("1.2.1", metadata.Element(ns + "version")!.Value);

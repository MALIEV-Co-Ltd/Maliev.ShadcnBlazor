...
public sealed class PackageArchiveTests
{
    ...
    public void ReleasePackageContainsOnlyTheExpectedPublicDistributionAssets()
    {
        ...
        try
        {
            ...
            var package = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.2.nupkg");
            var symbols = Path.Combine(output, "Maliev.ShadcnBlazor.1.0.2.snupkg");
            ...
            Assert.Equal("1.0.2", metadata.Element(ns + "version")!.Value);

...
public sealed class ThemeScenarioCatalogTests
{
    ...
    public void CatalogContainsThreeStableBilingualScenariosForEveryDocumentedComponent()
    {
        ...
        Assert.Equal(204, all.Select(value => value.English.Title).Distinct(StringComparer.Ordinal).Count());
        Assert.Equal(204, all.Select(value => value.Thai.Title).Distinct(StringComparer.Ordinal).Count());

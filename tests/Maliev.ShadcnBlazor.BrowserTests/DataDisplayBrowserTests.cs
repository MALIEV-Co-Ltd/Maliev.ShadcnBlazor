...
public sealed class DataDisplayBrowserTests(ShowcaseServerFixture server, PlaywrightFixture playwright)
{
    ...
    public async Task ChartKeyboardTooltipThemeDirectionAndResizeRemainUsable()
    {
        ...
        var points = surface.Locator("[data-slot='chart-point'][data-point='1']");
        await Assertions.Expect(points).ToHaveCountAsync(2);
        await points.First.HoverAsync();
        await Assertions.Expect(points.First).ToHaveAttributeAsync("data-active", "true");
        await Assertions.Expect(points.Last).ToHaveAttributeAsync("data-active", "true");
        var pointBox = await points.First.BoundingBoxAsync();
        Assert.NotNull(pointBox);
        Assert.InRange(Math.Abs(pointBox.Width - pointBox.Height), 0, 0.5);
        Assert.Equal(2, (await points.EvaluateAllAsync<string[]>("elements => elements.map(element => element.getAttribute('cy'))")).Distinct(StringComparer.Ordinal).Count());
